...
import { Template, TEMPLATE_ITEM_DATA_TYPES, TemplateItem } from "@/types/scanRule.types";
...
const TemplateFieldEditor = ({ fieldName, template, setData } : Props) => {
  const {
    ...
    isInspectionMode
  } = useInspection(INSPECTION_MODE.TAG_EXTRACTION, template.rootTag, uniqueCssSelectorOptions);
  ...
  const addItem = () => {
    setData({
      ...data,
      items: [...data.items, { name: "", content: "", dataType: TEMPLATE_ITEM_DATA_TYPES.TEXT, isOptional: true }]
    });
  };
